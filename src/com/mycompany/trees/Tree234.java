...
class Tree234
{
    ...
    private class Node
    {
        ...
        int insertItem(DataItem newItem)
        {
            ...
            {                                 //    examine items
                if(itemArray[j] != null)          // if item null not null,
                {                              // get its key
    ...
    public void sort(long[] array){
        ...
        arraySize = 0;
        sortArray(root, 0, array);
    }

    private void sortArray(Node node, int level, long[] sortedArray){
        ...
        for(int j=0; j<numItems+1; j++)
        {
            ...
            if(nextNode != null) {
                sortArray(nextNode, level + 1, sortedArray);

                if (node.getItem(j) != null)
                    sortedArray[arraySize++] = node.getItem(j).dData;
            }
            else {
                for (int k = 0; k < numItems; k++) {
                    sortedArray[arraySize++] = node.getItem(k).dData;
                }
                return;
            }
        }
...
class Tree234App
{
    public static void main(String[] args) throws IOException
    {
        ...
        long[] array = {37, 30, 34, 48, 60, 82, 5, 32, 33, 35, 40, 50, 53, 86};
        printArray(array);
        theTree.sort(array);
        printArray(array);
        theTree.displayTree();
